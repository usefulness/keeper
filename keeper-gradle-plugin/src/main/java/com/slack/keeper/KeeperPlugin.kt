 ...
 *   [JavaCompile] tasks and [org.jetbrains.kotlin.gradle.tasks.KotlinCompilationTask] tasks if available.
...
public class KeeperPlugin : Plugin<Project> {
    ...
    private fun resolveAndroidEmbeddedJar(
        ...
        // Platforms may ship as minor-versioned directories only (e.g. "android-37.0")
        val platformDir = File(platformsDir, "android-$compileSdkVersion").takeIf(File::exists)
            ?: platformsDir.listFiles()
                ...
                .maxByOrNull { it.name.substringAfterLast('.').toIntOrNull() ?: -1 }
    ...
    private fun wireClassesAndJarsFor(component: Component, taskProvider: TaskProvider<out KeeperJarTask>) {
        component.artifacts
...
private fun Configuration.proguardFiles(): FileCollection = artifactView(ArtifactType.FILTERED_PROGUARD_RULES)

private fun Configuration.artifactView(artifactType: ArtifactType): FileCollection = incoming
