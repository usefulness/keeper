...
internal fun File.newFile(path: String, block: (File.() -> Unit)? = null): File = File(this, path).apply {
    parentFile.mkdirs()
    block?.invoke(this)
}

internal fun File.newDir(path: String): File = File(this, path).apply { mkdirs() }
...
internal sealed class SourceFile(val name: String) {
    ...
    data class KotlinSourceFile(val fileSpec: FileSpec) : SourceFile(fileSpec.members.filterIsInstance<TypeSpec>().first().name!!) {
